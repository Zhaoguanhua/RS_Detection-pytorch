...
class PointRendRoIHeads(torch.nn.Module):
    __annotations__ = {
    ...
    def has_mask(self):
        if self.mask_coarse_head is None:
            return False
        if self.mask_point_head is None:
            return False
        return True
    ...
    def forward(self,
        ...
        if self.has_mask():
            ...
            mask_coarse_logits=self._forward_mask_coarse(features,mask_proposals)

            loss_mask = {}
            if self.training:
                assert targets is not None
                assert pos_matched_idxs is not None
                assert mask_coarse_logits is not None

                gt_masks = [t["masks"] for t in targets]
                gt_labels = [t["labels"] for t in targets]

                rcnn_loss_mask = maskrcnn_loss(
                    mask_coarse_logits, mask_proposals,
                    gt_masks, gt_labels, pos_matched_idxs)
                loss_mask = {
                    "loss_mask": rcnn_loss_mask
                }

                gt_masks=[gt_mask[idxs] for gt_mask,idxs in zip(gt_masks,pos_matched_idxs)]
                loss_mask_point=self._forward_mask_point(features,mask_coarse_logits,mask_proposals,mask_labels,gt_masks)

                loss_mask["loss_mask_point"]=loss_mask_point

            else:
                labels = [r["labels"] for r in result]

                gt_masks=[]
                mask_logits=self._forward_mask_point(features,mask_coarse_logits,mask_proposals,labels,gt_masks)
    ...
    def _forward_mask_point(self, features, mask_coarse_logits, instances,gt_classes,gt_masks):
        """
        ...
        if self.training:
            proposal_boxes = [x for x in instances]
            cat_gt_classes = torch.cat(gt_classes,dim=0)
            with torch.no_grad():
                point_coords = get_uncertain_point_coords_with_randomness(
                    mask_coarse_logits,
                    lambda logits: calculate_uncertainty(logits, cat_gt_classes),
            ...
            return roi_mask_point_loss(
                    point_logits, instances, point_coords_wrt_image,gt_classes,gt_masks
                )
        else:
            pred_boxes = [x for x in instances]
            pred_classes = torch.cat(gt_classes,dim=0)
